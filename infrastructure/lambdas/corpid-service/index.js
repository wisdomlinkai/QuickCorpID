...
// types.ts
var CorpIDConnectionStatusSchema = external_exports.enum([
  "pending",
  "active",
  "expired",
  "revoked"
]);
var CorpIDEnvironmentSchema = external_exports.enum(["sandbox", "production"]);
var CorpIDTokenResponseSchema = external_exports.object({
...
var CorpIDUserInfoSchema = external_exports.object({
  corp_id: external_exports.string(),
  company_name: external_exports.string(),
  br_number: external_exports.string(),
  cr_number: external_exports.string().optional(),
  authorised_representatives: external_exports.array(external_exports.object({
    name: external_exports.string(),
    email: external_exports.string(),
    role: external_exports.string()
  })).optional()
});
var CorpIDDocumentSchema = external_exports.object({
  document_id: external_exports.string(),
  document_type: external_exports.string(),
  document_name: external_exports.string(),
  created_at: external_exports.string(),
  hash: external_exports.string().optional(),
  signed: external_exports.boolean().optional()
});
var CorpIDConnectionDBSchema = external_exports.object({
  id: external_exports.string().uuid(),
  org_id: external_exports.string().uuid(),
  corp_id: external_exports.string(),
  status: CorpIDConnectionStatusSchema,
  access_token_encrypted: external_exports.string().optional(),
  refresh_token_encrypted: external_exports.string().optional(),
  token_expires_at: external_exports.date().optional(),
  last_synced_at: external_exports.date().optional(),
  created_at: external_exports.date(),
  updated_at: external_exports.date()
});
var ConnectCorpIDRequestSchema = external_exports.object({
  org_id: external_exports.string().uuid(),
  environment: CorpIDEnvironmentSchema,
  auth_code: external_exports.string().optional(),
  state: external_exports.string().optional()
});
var GenerateQRCodeRequestSchema = external_exports.object({
  org_id: external_exports.string().uuid(),
  purpose: external_exports.enum(["login", "signing", "document_wallet"]),
  document_id: external_exports.string().uuid().optional(),
  callback_url: external_exports.string().url().optional(),
  expires_in_seconds: external_exports.number().min(60).max(3600).default(300)
});
var SyncDocumentWalletRequestSchema = external_exports.object({
  org_id: external_exports.string().uuid(),
  document_types: external_exports.array(external_exports.string()).optional()
});
var GetSigningStatusRequestSchema = external_exports.object({
  org_id: external_exports.string().uuid(),
  signing_request_id: external_exports.string().uuid()
});
var InitiateSigningRequestSchema = external_exports.object({
  org_id: external_exports.string().uuid(),
  document_id: external_exports.string().uuid(),
  signers: external_exports.array(external_exports.object({
    email: external_exports.string().email(),
    name: external_exports.string(),
    role: external_exports.string().optional()
  })),
  callback_url: external_exports.string().url().optional(),
  expires_in_hours: external_exports.number().min(1).max(168).default(72)
});
var RefreshTokenRequestSchema = external_exports.object({
  org_id: external_exports.string().uuid()
});
var CorpIDConnectionResponseSchema = external_exports.object({
  connection_id: external_exports.string().uuid(),
  corp_id: external_exports.string(),
  company_name: external_exports.string(),
  br_number: external_exports.string(),
  cr_number: external_exports.string().optional(),
...
var QRCodeResponseSchema = external_exports.object({
  qr_code_id: external_exports.string().uuid(),
  qr_code_url: external_exports.string(),
  deep_link: external_exports.string().optional(),
  expires_at: external_exports.string(),
  status: external_exports.enum(["pending", "scanned", "completed", "expired"])
});
var SigningStatusResponseSchema = external_exports.object({
  signing_request_id: external_exports.string().uuid(),
  status: external_exports.enum(["pending", "in_progress", "completed", "expired", "cancelled"]),
  signers: external_exports.array(external_exports.object({
    email: external_exports.string(),
    name: external_exports.string(),
    status: external_exports.enum(["pending", "signed", "declined"]),
    signed_at: external_exports.string().optional()
  })),
  completed_at: external_exports.string().optional(),
  document_url: external_exports.string().optional()
});

// database.ts
var import_pg = require("pg");
var import_client_secrets_manager = require("@aws-sdk/client-secrets-manager");
var secretsClient = new import_client_secrets_manager.SecretsManagerClient({ region: process.env.AWS_REGION || "ap-southeast-1" });
var pool = null;
...
async function createCorpIDConnection(orgId, corpId, status, accessTokenEncrypted, refreshTokenEncrypted, tokenExpiresAt) {
...
async function getCorpIDConnectionByOrgId(orgId) {
  const client = await getPool();
  const result = await client.query(
...
async function updateCorpIDConnectionTokens(connectionId, accessTokenEncrypted, refreshTokenEncrypted, tokenExpiresAt) {
  const client = await getPool();
  const result = await client.query(
...
async function deleteCorpIDConnection(connectionId) {
  const client = await getPool();
  await client.query(
...
async function createSigningRequest(orgId, documentId, signers, expiresAt) {
  const client = await getPool();
  const result = await client.query(
...
var kmsClient = new import_client_kms.KMSClient({ region: process.env.AWS_REGION || "ap-southeast-1" });
var secretsClient2 = new import_client_secrets_manager2.SecretsManagerClient({ region: process.env.AWS_REGION || "ap-southeast-1" });
async function encryptData(plaintext) {
  const kmsKeyId = process.env.KMS_KEY_ID;
  if (!kmsKeyId) {
    throw new Error("KMS_KEY_ID environment variable not set");
  }
  const command = new import_client_kms.EncryptCommand({
    KeyId: kmsKeyId,
...
async function decryptData(ciphertext) {
  const command = new import_client_kms.DecryptCommand({
...
async function encryptCorpIDToken(token) {
  try {
    const encryptedToken = await encryptData(token);
    return {
      encryptedToken,
      encryptedKey: ""
      // Would contain the encrypted CEK in production
    };
  } catch (error) {
...
async function decryptCorpIDToken(encryptedToken) {
  try {
...
var CorpIDClient = class {
   ...
   */
  async generateQRCode(accessToken, purpose, options) {
   ...
   */
  async checkQRCodeStatus(accessToken, qrCodeId) {
   ...
   */
  async syncDocumentWallet(accessToken, documentTypes) {
   ...
   */
  async initiateSigning(accessToken, documentId, signers, options) {
   ...
   */
  async checkSigningStatus(accessToken, signingRequestId) {
   ...
   * Revoke connection
   */
  async revokeConnection(accessToken) {
  ...
  try {
    ...
    switch (true) {
      ...
      case (method === "GET" && path === "/corpid/qrcode/{qrCodeId}"):
        result = await handleCheckQRCodeStatus(queryParams.qrCodeId, userId);
        break;
      // Sync Document Wallet
      ...
      case (method === "GET" && path === "/corpid/connection"):
        result = await handleGetConnection(queryParams.org_id, userId);
        break;
      // Refresh token
      ...
      case (method === "DELETE" && path === "/corpid/connection"):
        result = await handleDisconnectCorpID(queryParams.org_id, userId);
        break;
      // Initiate signing
...
async function handleConnectCorpID(body, userId) {
  ...
  const encryptedRefreshToken = tokenResponse.refresh_token ? await encryptCorpIDToken(tokenResponse.refresh_token) : null;
  const tokenExpiresAt = new Date(Date.now() + tokenResponse.expires_in * 1e3);
...
async function handleGenerateQRCode(body, userId) {
...
async function handleCheckQRCodeStatus(qrCodeId, userId) {
  return {
    qr_code_id: qrCodeId,
    status: "pending"
  };
}
async function handleSyncDocumentWallet(body, userId) {
...
async function handleGetConnection(orgId, userId) {
...
async function handleRefreshToken(body, userId) {
...
async function handleDisconnectCorpID(orgId, userId) {
  const connection = await getCorpIDConnectionByOrgId(orgId);
  if (!connection) {
    return { success: true };
  }
...
async function handleInitiateSigning(body, userId) {
  ...
  const expiresAt = new Date(Date.now() + request.expires_in_hours * 60 * 60 * 1e3);
...
async function handleGetSigningStatus(queryParams, userId) {
